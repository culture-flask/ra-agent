...
def _effective_cfg(ctx: WorkflowContext, user_id: str):
...
def _role_cfg(ctx: WorkflowContext, state: BrainstormState, agent_id: str):
...
async def _llm_text(ctx: WorkflowContext, state: BrainstormState,
                   system_prompt: str, human_content: str,
                   temperature: float = 0.2) -> tuple[str, int]:
    """控制类 LLM 调用（prepare / moderator）：不向前端流式透传，
    只需最终文本。返回 (文本, token用量)。失败上抛，由调用方降级。"""
    sid, uid = state["session_id"], state["user_id"]
    payload = [SystemMessage(content=system_prompt),
               HumanMessage(content=human_content)]
    cfg = _effective_cfg(ctx, uid)
    if cfg is not None:
        resp, usage, stopped = await native_round(
            cfg, payload, None, temperature, sid,
            token_event="bs_internal", reasoning_event="bs_internal",
    ...
    model = await asyncio.to_thread(ctx.llm_service.get_chat_model, uid,
                                    temperature=temperature)
    resp = await model.ainvoke(payload)
    return str(resp.content or ""), 0


async def _langchain_stream(model, msgs, sid: str, extra: dict):
    """langchain 回退路径的一轮流式（镜像主图 _langchain_round，
    事件换 bs_token 并带 agent 字段）。"""
    resp = None
    stopped = False
    async for chunk in model.astream(msgs):
        ...
        if isinstance(text, str) and text:
            emit("bs_token", {"content": text, **extra})
        elif isinstance(text, list):
            for part in text:
                if isinstance(part, dict) and part.get("text"):
                    emit("bs_token", {"content": part["text"], **extra})
    if resp is None:
        resp = AIMessage(content="")
    return resp, None, stopped


async def _agent_speak(ctx: WorkflowContext, state: BrainstormState,
                       agent_id: str, system_prompt: str, human_content: str,
                       temperature: float, tools: list | None,
                       tool_loop_max: int,
                       cfg=None) -> tuple[str, int, bool]:
    """一个 agent 的一次流式发言（含 generate⇄tool_executor 式子循环）。

    返回 (发言全文, 累计token, stopped)。所有事件带 {"agent": agent_id}。
    ...
    if cfg is None:                         # 调用方未解析（测试/内部兜底）
        cfg = _role_cfg(ctx, state, agent_id)
    ...
    while round_idx < rounds + 1:           # +1：工具预算耗尽后的强制收尾轮
        ...
        if cfg is not None:
            resp, usage, stopped = await native_round(
                ...
                token_event="bs_token", reasoning_event="bs_reasoning",
                emit_extra=extra)
        else:
            ...
            resp, usage, stopped = await _langchain_stream(model, msgs, sid, extra)
        round_idx += 1
        ...
        if calls and not last_round:
            ...
            msgs.append(resp)
            for call in calls:
                emit("bs_tool_start", {"agent": agent_id, "name": call["name"],
                                       "args": call["args"]})
                try:
                ...
                # 落日志 + 随 bs_tool_end 推给前端（红 chip），否则用户只能
                # 从模型转述里猜"基础设施错误"（真实教训：嵌入端点宕机窗口）
                err = out.get("error") if isinstance(out, dict) else None
                if err:
                    logger.warning("bs tool %s failed for %s: %s",
                                   call["name"], agent_id, str(err)[:300])
                emit("bs_tool_end", {"agent": agent_id, "name": call["name"],
        ...
        if empty_retried >= EMPTY_RETRY_MAX:
            return text_out, used, stopped
        empty_retried += 1
        logger.warning("bs agent %s empty completion, retry %d/%d",
